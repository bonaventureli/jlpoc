...
Can_FrameType CANRecDataBuff1[CAN_FIFO_LENTH];
Can_FrameType CANRecDataBuff2[CAN_FIFO_LENTH];
Can_FrameType CANRecDataBuff3[CAN_FIFO_LENTH];
...
// Functions
//============================================================================
S_RING_QUEUE CANQueue1;
S_RING_QUEUE CANQueue2;
S_RING_QUEUE CANQueue3;

void BspCanQueueInit(void)
{
	UtilRingQueueInit(&CANQueue1,&CANRecDataBuff1[0],CAN_FIFO_LENTH);
	UtilRingQueueInit(&CANQueue2,&CANRecDataBuff2[0],CAN_FIFO_LENTH);
	UtilRingQueueInit(&CANQueue3,&CANRecDataBuff3[0],CAN_FIFO_LENTH);
}
...
void CanPinConfig(void)
{
...
#if BOARD_DEMO // Adapt to DemoBoard (if use to Develop,there are some wrong of Sent)
	// CAN0 ------------------------------------------------------	 
  ...
  PM0 |= 1<<1;
  PMC0 |= 0x00003;
#elif BOARD_DEVELOP  // Adapt to DevelopBoard
	/* P10_0 (CAN0RX) */
  PFCAE10 &= 0xFFFEU;
  PFCE10  &= 0xFFFEU;
  PFC10   |= 0x0001U;
...
void INTRCANGRECC_interruptISR( void )
{
	...
	while(RSCAN0.RFSTS0.UINT8[LH]  != 0)  // The receive FIFO buffer contains unread message.
	{
		...
		UtilRingQueuePush(&CANQueue1,pRxBuffer[0]);
	...
	while(RSCAN0.RFSTS1.UINT8[LH]  != 0)  // The receive FIFO buffer contains unread message.
	{
		...
		UtilRingQueuePush(&CANQueue2,CANRecData2);
	...
	while(RSCAN0.RFSTS2.UINT8[LH]  != 0)  // The receive FIFO buffer contains unread message.
	{
		...
		UtilRingQueuePush(&CANQueue3,CANRecData3);
...
/******************************************************************************
** Function:    CANDataSent
** Description: Set a frame of data in channel n
** Parameter:   CANchn the channel of use
								IDStd Identifer
**              Data point of Data
**              DataLen Lenth of data
** Return:      The result of sent
******************************************************************************/
static Can_RtnType CANDataSent(uint8_t CANchn,uint32_t IDStd, uint8_t *Data,uint8_t DataLen)
{
  Can_RtnType	Retval;
	...
	Retval = Can_ChTrmByTxBuf(CANchn,TransBuffNum+1,&TxFrame);  // Sent Data
	TransBuffNum++;
	if(TransBuffNum >= 15)
	{
		TransBuffNum = 0;
...
/******************************************************************************
** Function:    BspCANSentA
** Description: Set a frame of data in channel A
** Parameter:   IDStd Identifer
**              Data point of Data
**              DataLen Lenth of data
** Return:      The result of sent
******************************************************************************/
Can_RtnType BspCANSentA(uint32_t IDStd, uint8_t *Data,uint8_t DataLen)
{
	Can_RtnType Retval;
	Retval = CANDataSent(1,IDStd,Data,DataLen);
	return Retval;
}

/******************************************************************************
** Function:    BspCANSentB
** Description: Set a frame of data in channel A
** Parameter:   IDStd Identifer
**              Data point of Data
**              DataLen Lenth of data
** Return:      The result of sent
******************************************************************************/
Can_RtnType BspCANSentB(uint32_t IDStd, uint8_t *Data,uint8_t DataLen)
{
	Can_RtnType Retval;
	Retval = CANDataSent(2,IDStd,Data,DataLen);
	return Retval;
}

/******************************************************************************
** Function:    BspCANSentC
** Description: Set a frame of data in channel A
** Parameter:   IDStd Identifer
**              Data point of Data
**              DataLen Lenth of data
** Return:      The result of sent
******************************************************************************/
Can_RtnType BspCANSentC(uint32_t IDStd, uint8_t *Data,uint8_t DataLen)
{
	Can_RtnType Retval;
	Retval = CANDataSent(3,IDStd,Data,DataLen);
	return Retval;
}

/******************************************************************************
* Function Name : BspCANReceiveA
* Description   : CAN A Bus Receive
* Argument      : canId  Data DataLen;
* Return Value  : Is Received or not
******************************************************************************/
uint8_t BspCANReceiveA(uint32_t* canId,uint8_t* Data,uint8_t* DataLen)
{
	uint8_t Retval = 0;
	...
	{
		gFlagNewData1 = 0;
		*canId = CANRecData1.ID;
		*DataLen = CANRecData1.DLC; 
		memcpy(Data,&CANRecData1.DB[0],CANRecData1.DLC);
		Retval = 1;
	}
	return Retval;
}
...
/******************************************************************************
* Function Name : BspCANReceiveB
* Description   : CAN B Bus Receive
* Argument      : canId  Data DataLen;
* Return Value  : Is Received or not
******************************************************************************/
uint8_t BspCANReceiveB(uint32_t* canId,uint8_t* Data,uint8_t* DataLen)
{
	uint8_t Retval = 0;
	...
	{
		gFlagNewData2 = 0;
		*canId = CANRecData2.ID;
		*DataLen = CANRecData2.DLC; 
		memcpy(Data,&CANRecData2.DB[0],CANRecData2.DLC);
		Retval = 1;
	}
	return Retval;
}
...
/******************************************************************************
* Function Name : BspCANReceiveC
* Description   : CAN C Bus Receive
* Argument      : canId  Data DataLen;
* Return Value  : Is Received or not
******************************************************************************/
uint8_t BspCANReceiveC(uint32_t* canId,uint8_t* Data,uint8_t* DataLen)
{
	uint8_t Retval = 0;
	...
	{
		gFlagNewData3 = 0;
		*canId = CANRecData3.ID;
		*DataLen = CANRecData3.DLC; 
		memcpy(Data,&CANRecData3.DB[0],CANRecData3.DLC);
		Retval = 1;
	}
	return Retval;
}
...
/******************************************************************************
* Function Name : BspCANReceiveQueueA
* Description   : A CAN Bus Receive
* Argument      : canId  Data DataLen;
* Return Value  : Is Received or not
******************************************************************************/
uint8_t BspCANReceiveQueueA(uint32_t* canId,uint8_t* Data,uint8_t* DataLen)
{
	Can_FrameType CANRecData;
	uint8_t Retval = 0;
	
	if(TRUE == UtilRingQueuePop(&CANQueue1,&CANRecData))
	{
		*canId = CANRecData.ID;
		*DataLen = CANRecData.DLC; 
		memcpy(Data,&CANRecData.DB[0],CANRecData.DLC);
		Retval = 1;
	}
	return Retval;
}
...
/******************************************************************************
* Function Name : BspCANReceiveQueueB
* Description   : B CAN Bus Receive
* Argument      : canId  Data DataLen;
* Return Value  : Is Received or not
******************************************************************************/
uint8_t BspCANReceiveQueueB(uint32_t* canId,uint8_t* Data,uint8_t* DataLen)
{
	Can_FrameType CANRecData;
	uint8_t Retval = 0;
	
	if(TRUE == UtilRingQueuePop(&CANQueue2,&CANRecData))
	{
		*canId = CANRecData.ID;
		*DataLen = CANRecData.DLC; 
		memcpy(Data,&CANRecData.DB[0],CANRecData.DLC);
...
/******************************************************************************
* Function Name : BspCANReceiveQueueC
* Description   : C CAN Bus Receive
* Argument      : canId  Data DataLen;
* Return Value  : Is Received or not
******************************************************************************/
uint8_t BspCANReceiveQueueC(uint32_t* canId,uint8_t* Data,uint8_t* DataLen)
{
	Can_FrameType CANRecData;
	uint8_t Retval = 0;
	
	if(TRUE == UtilRingQueuePop(&CANQueue3,&CANRecData))
	{
		*canId = CANRecData.ID;
		*DataLen = CANRecData.DLC; 
		memcpy(Data,&CANRecData.DB[0],CANRecData.DLC);
